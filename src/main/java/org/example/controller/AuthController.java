...
@Tag(
    name = "Authentication",
    description = "Endpoints for Login, Register, Google Auth, and Token Management")
public class AuthController {
  ...
  @Operation(
      ...
      description = "Creates a new local user account and returns JWT tokens.")
  ...
  @Operation(
      ...
      description = "Authenticates a local user and returns JWT tokens.")
  @PostMapping("/login")
  ...
  @Operation(
      ...
      description = "Exchanges a Google Auth Code for JWT tokens and setup offline access.")
  @PostMapping("/google")
  public ResponseWrapper<AuthResponse> googleAuth(@RequestBody GoogleAuthRequest request)
      throws Exception {
  ...
  public ResponseWrapper<Void> logout(@AuthenticationPrincipal UserDetails userDetails) {
    // userDetails comes from the JWT Filter. If token is invalid, Filter blocks it before reaching
    // here.
    if (userDetails != null) {
