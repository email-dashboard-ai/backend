...
public class EmailEmbeddingServiceImpl implements EmailEmbeddingService {
  ...
  public int generateMissingEmbeddings(String userEmail) {
    ...
    log.info(
        "Query returned {} emails without embeddings for user: {}",
        emailsWithoutEmbeddings.size(),
        userEmail);

    if (emailsWithoutEmbeddings.isEmpty()) {
      log.warn(
          "No emails without embeddings found for user: {}. Either all have embeddings or user has no emails.",
          userEmail);
      return 0;
    }

    log.info(
        "Found {} emails without embeddings for user: {}",
        emailsWithoutEmbeddings.size(),
        userEmail);
    ...
    for (SyncedEmail email : emailsWithoutEmbeddings) {
      try {
        ...
        if (embedding != null) {
          ...
          if (processed % 10 == 0) {
            log.debug(
                "Generated embeddings for {}/{} emails", processed, emailsWithoutEmbeddings.size());
          }
        } else {
        ...
        log.error(
            "Error generating embedding for email {}: {}", email.getMessageId(), e.getMessage());
      }
    }

    log.info(
        "Generated embeddings for {}/{} emails for user: {}",
        processed,
        emailsWithoutEmbeddings.size(),
        userEmail);
  ...
  public int generateAllMissingEmbeddings() {
    ...
    log.info(
        "Found {} emails without embeddings across all users", allEmailsWithoutEmbeddings.size());
    ...
    for (SyncedEmail email : allEmailsWithoutEmbeddings) {
      try {
        ...
        if (embedding != null) {
          ...
          if (processed % 50 == 0) {
            log.info(
                "Generated embeddings for {}/{} emails",
                processed,
    ...
    log.info(
        "Completed batch embedding generation: {}/{} emails processed",
        processed,
        allEmailsWithoutEmbeddings.size());
