...
public class KanbanColumnServiceImpl implements KanbanColumnService {
    ...
    public KanbanColumnResponse createColumn(String userEmail, KanbanColumnRequest request)
        ...
        if (kanbanColumnRepository.existsByUserAndColumnId(user, request.columnId())) {
            throw new ValidationException("Column with ID '" + request.columnId() + "' already exists. Please choose a different name or delete the existing column first.");
        }
        ...
        try {
            ...
            log.error("Failed to create/link Gmail label for column '{}': {}", request.columnId(), e.getMessage());
            throw new IOException("Failed to create/link Gmail label: " + e.getMessage(), e);
        }

        column = kanbanColumnRepository.save(column);
        log.info("Created Kanban column '{}' for user '{}' with Gmail sync", column.getColumnId(), userEmail);
    ...
    public KanbanColumnResponse updateColumn(
        ...
        if (nameChanged && column.getGmailLabelId() != null) {
            try {
                Label updatedLabel = googleEmailStrategy.renameGmailLabel(user, column.getGmailLabelId(),
                        request.name());
                column.setGmailLabelName(updatedLabel.getName());
                log.info(
                        "Renamed Gmail label to '{}' for column '{}'", updatedLabel.getName(), column.getColumnId());
            } catch (IOException e) {
    ...
    public void deleteColumn(String userEmail, Long columnId) throws IOException {
        ...
        if (column.getGmailLabelId() != null) {
            try {
                ...
                log.info("Deleted Gmail label '{}' for column '{}'", column.getGmailLabelName(), column.getColumnId());
            } catch (IOException e) {
    ...
    public void moveEmailToColumn(String userEmail, String emailId, String targetColumnId) {
        ...
        KanbanColumn targetColumn = kanbanColumnRepository.findByUserAndColumnId(user, targetColumnId)
        ...
        if ("inbox".equals(normalizedId))
            status = KanbanStatus.INBOX;
        else if ("todo".equals(normalizedId))
            status = KanbanStatus.TO_DO;
        else if ("in_progress".equals(normalizedId))
            status = KanbanStatus.IN_PROGRESS;
        else if ("done".equals(normalizedId))
            status = KanbanStatus.DONE;
        else
            status = KanbanStatus.IN_PROGRESS; // Default for custom columns
        ...
        try {
            ...
            log.info("📧 Moving email {} to column '{}' (ID: {})",
                    emailId, targetColumn.getName(), targetColumnId);

            // Validate that target column has a Gmail label (required now)
            if (targetColumn.getGmailLabelId() == null) {
                throw new ValidationException("Target column '" + targetColumn.getName() +
            ...
            log.info("  ✓ Will ADD label: '{}' ({})", targetColumn.getName(), targetColumn.getGmailLabelId());
