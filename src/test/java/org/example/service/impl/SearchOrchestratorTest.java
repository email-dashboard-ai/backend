...
class SearchOrchestratorTest {
  ...
  void resolve_ShouldReturnInternal_WhenRequestIsNull() {
    ...
    assertThat(result.getStrategy()).isEqualTo(SearchOrchestrator.Strategy.INTERNAL);
    assertThat(result.getFuzzyQuery()).isEmpty();
  }
  ...
  void resolve_ShouldReturnInternal_WhenOnlyBodyFieldPresent() {
    ...
    assertThat(result.getStrategy()).isEqualTo(SearchOrchestrator.Strategy.INTERNAL);
    assertThat(result.getGmailQuery()).isNull();
    assertThat(result.getFuzzyQuery()).isEqualTo("search term");
  }
  ...
  void resolve_ShouldReturnHybrid_WhenBothFieldsPresent() {
    ...
    assertThat(result.getStrategy()).isEqualTo(SearchOrchestrator.Strategy.HYBRID);
    assertThat(result.getGmailQuery()).isEqualTo("from:test@example.com");
    assertThat(result.getFuzzyQuery()).isEqualTo("search term");
  }
}
